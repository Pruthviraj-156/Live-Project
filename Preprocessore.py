...
# Fatching the date features
def fetch_time_features(df):
        df["Date"] =pd.to_datetime(df["Date"])
        df["Year"] = df["Date"].dt.year
        df["Month"] = df["Date"].dt.month
        df["Day"] = df["Date"].dt.day
        # For the financial month
...
def fetch_top_revenue_retailers(df):
        retailers_revenue = df[["Retailer", "Amount"]].groupby("Retailer").sum().reset_index().sort_values(by = "Amount", ascending = True)
        ...
        retailers = pd.DataFrame(data={
                "percentage revenue":percentages, 
                "retailer count":retailers_count 
        })
        return retailers
# Company revenue
